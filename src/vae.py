...
class Encoder(nn.Module):
    def __init__(self, options, dropout_p=0.5, q_dims=[11472, 600, 200]):
    ...
    def forward(self, x):
        ...
        x = self.tanh(x)
        x = self.linear_2(x)
        mu_q, logvar_q = torch.chunk(x, chunks=2, dim=1) # what does this do, output two different layers
        return mu_q, logvar_q


class Decoder(nn.Module):
    def __init__(self, options, p_dims=[200, 600, 11472]):
...
class MultiVAE(nn.Module):
    def __init__(self, cuda2=True, weight_decay=0.0, dropout_p=0.5, q_dims=[11472, 600, 200], p_dims=[200, 600, 11472], n_conditioned=0):
    ...
    def forward(self, x, c):
        ...
        if self.training:
            ...
            sampled_z = torch.normal(mu_q, std_q)
